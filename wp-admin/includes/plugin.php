...
/**
 * Check the custom-plugins directory and retrieve all custom-plugin files with any plugin data.
 *
 * WordPress only includes custom-plugin files in the base custom-plugins directory (wp-content/custom-plugins).
 *
 * @since 3.0.0
 * @return array Key is the custom-plugin file path and the value is an array of the custom-plugin data.
 */
function get_custom_plugins() {
	$wp_plugins = array();
	// Files in wp-content/custom-plugins directory
	$plugin_files = array();

	if ( ! is_dir( WPCT_PLUGIN_DIR ) )
		return $wp_plugins;
	if ( $plugins_dir = @ opendir( WPCT_PLUGIN_DIR ) ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
			if ( substr( $file, -4 ) == '.php' )
				$plugin_files[] = $file;
		}
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( !is_readable( WPCT_PLUGIN_DIR . "/$plugin_file" ) )
			continue;

		$plugin_data = get_plugin_data( WPCT_PLUGIN_DIR . "/$plugin_file", false, false ); //Do not apply markup/translate as it'll be cached.
	...
	if ( isset( $wp_plugins['index.php'] ) && filesize( WPCT_PLUGIN_DIR . '/index.php') <= 30 ) // silence is golden
 ...
 *
 * Plugins in the customplugins/ folder can't be "activated," so this function will
