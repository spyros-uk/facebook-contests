 ...
 *
 * The default directory is wp-content/custom-plugins. To change the default
 * directory manually, define `WPCT_PLUGIN_DIR` and `WPCT_PLUGIN_URL`
 * in wp-config.php.
 *
 * @since 3.0.0
 * @access private
 ...
 */
function wp_get_ct_plugins() {
	$ct_plugins = array();
	if ( !is_dir( WPCT_PLUGIN_DIR ) )
		return $ct_plugins;
	if ( ! $dh = opendir( WPCT_PLUGIN_DIR ) )
		return $ct_plugins;
	while ( ( $plugin = readdir( $dh ) ) !== false ) {
		if ( substr( $plugin, -4 ) == '.php' )
			$ct_plugins[] = WPCT_PLUGIN_DIR . '/' . $plugin;
	}
	closedir( $dh );
	sort( $ct_plugins );
