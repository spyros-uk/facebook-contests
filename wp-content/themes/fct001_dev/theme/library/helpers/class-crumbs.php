...
class SDP_Crumbs {
				...
				<?php foreach ( $breadcrumbs_items as $item ) : ?>
					...
					<li class="crumbs__list-item">
						<?php
							...
							SDP_Html::element( $tag, $attrs, $item['text'] );
						?>
					</li>
	...
	public static function get_crumbs_array( $theme_location = 'primary', $options = array() ) {
		...
		if ( $current_item['id'] && ( $parents = SDP_Menu::get_parent_menu_items( $theme_location, $current_item['id'] ) ) ) {
		...
		return apply_filters( 'SDP_crumbs_array', $crumbs, $theme_location, $options );
	}
